...
def test_get_all_tasks_of_week(logger):
    ...
    with allure.step("Creating a new week"):
        week = add_week(week_number=1, program_id=id_program, headers={"Authorization": token})
        id_week = week.json()["id"]

    with allure.step("creating a new task"):
        new_task = create_new_task(weekID=id_week, headers={"Authorization": token})
    ...
    with allure.step("Delete new certain task by id"):
        delete_t = delete_new_task_by_id(task_id=id_task, headers={"Authorization": token})
        print(delete_t.text)

    with allure.step("Delete new certain week by id"):
        delete_w = delete_week(week_id=id_week, headers={"Authorization": token})
        print(delete_w.text)
